...
from agents.peract_diffusion.perceiver_lang_io import PerceiverVoxelLangEncoder
from agents.peract_diffusion.qattention_peract_bc_agent import QAttentionPerActBCAgent
from agents.peract_diffusion.qattention_stack_agent import QAttentionStackAgent
...
def create_agent(cfg: DictConfig):
    ...
    for depth, vox_size in enumerate(cfg.method.voxel_sizes):
        ...
        perceiver_encoder = PerceiverVoxelLangEncoder(
            ...
            voxel_size=vox_size,
            initial_dim = 3 + 3 + 1 + 3 + 10, #combine with noised voxel channel
            low_dim_size=4,
            layer=depth,
